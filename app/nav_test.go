...
func TestSelectAdjacent(t *testing.T) {
	...
	// Down from no selection lands on the first card.
	a.SelectAdjacent(1)
	if it, ok := a.scene.PreviewItem(); !ok || it.ID != "0" {
		t.Fatalf("first select = %+v ok=%v", it, ok)
	}
	...
	a.SelectAdjacent(-1)
	if it, _ := a.scene.PreviewItem(); it.ID != "0" {
